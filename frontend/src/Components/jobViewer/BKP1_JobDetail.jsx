...
const JobDetail = (props) => {
  ...
  // En modo jobs/edit (sin props.cot) esperamos a que el fetch de cotizaciones
  // previas termine antes de montar las partes, para que sus useState lean la
  // imposición ya cargada desde el primer render (igual que hace props.cot).
  const [cotizationsLoaded, setCotizationsLoaded] = useState(!!props.cot);
  ...
  const PartDetail = (part) => {
    const [usePoses, setPoses] = useState(
      props.cot 
      ? props.cot?.impositionData[part._id]?.Poses 
      : null,
    );
    const [useImpoData, setImpoData] = useState(
      props.cot 
      ? props.cot?.impositionData[part._id]?.impositionData : previousCotizations.length > 0 
        ? previousCotizations[0]?.data.impositionData[part._id]?.impositionData 
        : null,
    );
    const [imposed, setImposed] = useState(props.cot ? true : previousCotizations.length > 0 ? true : false);
    ...
    useEffect(() => {
      console.log("Cotizacion previa: ", previousCotizations);
      if (usePoses !== null) {
        setImposed(false);
        setData({
      ...
      <Card elevation={16}>
        ...
        <CardContent>
          ...
          {cotizationsLoaded ? (
            ...
            <Spinner title="Cargando imposición..." />
          )}
          {job.Finishing && (
            <Item>
              <FinishingListAuto
